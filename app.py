...
def get_maps():
    ***REMOVED***consider anything in gql_map as searchable, make all keys like
    ***REMOVED***project_name ==> project.name to abide by UI search rules
...
def get_cases():
    ...
    if not request.args:
        return 'placeholder' ***REMOVED***return a value when the cart page sends no args
    ...
    ***REMOVED***Processing autocomplete here as well as finding counts for the set category
    if(request.args.get('facets') and not request.args.get('expand')):
        ...
        query = ""
        if not request.args.get('facets'): ***REMOVED***advanced search
            query = {'query':ac_gql.format(filters,size,from_num,order,"")}
        else: ***REMOVED***facet search
        ...
        if save_query and save_query == 'yes':
            if 'HTTP_X_CSRFTOKEN' in request.environ:
                ...
                ***REMOVED***dislays in the advanced query input box. 
                ...
                save_query_sample_data(request.environ['HTTP_X_CSRFTOKEN'],request.environ['HTTP_REFERER'],query,re.search(r'"total":(\d+)',data).group(1))
...
def login():
    ...
    if request.method == 'POST':
        ...
        ***REMOVED***Now hit MySQL to see if these are valid 
...
def get_files():
    ...
    query = ""
    if not request.args.get('filters'): ***REMOVED***handle cart load
        cart_str = '''
            sample_count(cy:"{0}")
        '''
    ...
    save_query = request.args.get('save')
    if save_query and save_query == 'yes':
        save_query_file_data(request.environ['HTTP_REFERER'],re.search(r'"total":(\d+)',data).group(1))
...
def get_annotation():
    return 'placeholder' ***REMOVED***trimmed endpoint from GDC, not using for now
...
def get_manifest():
    string = 'file_id\tmd5\tsize\turls\tsample_id' ***REMOVED***header

...
def get_urls(sample_id):
    ...
    for x in range(0,len(res)):
        for key in res[0].keys(): ***REMOVED***iterate over all node paths traversed
...
def extract_url_info(urls_node):
    ...
    if re.match('.*http.*', urls_node):
        ...
        fn = name_and_url.group(2).replace("/",".") ***REMOVED***making the file name and some of its path pretty
        fi = name_and_url.group(1) ***REMOVED***File ID can just be our URL
    else:
        fn = "none"
        fi = "none"
    res.append(fn)
...
def get_files(sample_id):
    ...
    for x in range(0,len(res)): ***REMOVED***iterate over each unique path
        for key in res[0].keys(): ***REMOVED***iterate over each unique node in the path
            dt = res[0][key]['subtype']
            df = res[0][key]['format']
            ac = "open" ***REMOVED***again, default to accommodate current GDC format
            fs = res[0][key]['size']
